...
namespace EmployeeApp.Helper
{
    public class RandomEmployee
    {
        ...
        private static string[] lastNames = { "Abraham", "Campbell", "Ellison", "Henderson", "Johnston" },
            streets = { "2708 Adonais Way", "4154 Cherry Tree Drive", "3466 WilmarFarm Road", "1949 Jadewood Drive", "501 Blane Street" },
            cities = { "Atlanta", "Jacksonville", "Lanham", "Wheatfield", "FairviewHeights" },
        ...
        private static Address generateAddress()
        {
            return new Address()
            {
                City = generateElementOf(cities),
                State =generateElementOf(states),
                Street = generateElementOf(streets),
                Zip = generateZips()
            };
        }
        public static Employee generateEmployee()
        {
            ...
            if (generateJob() == 0)
               ...
               gender, generateAddress(),
                ...
                employee = new Tester(Id, firstName, lastName, Convert.toEmailFormat(firstName, lastName),
                    Convert.toHungarianPhoneFormatOf(generateElementOf(phones)), gender,
                    generateAddress(), rnd.NextDouble() * 1000.0, rnd.NextDouble() * 13.0);
            else
                employee = new SystemEngineer(Id, firstName, lastName, Convert.toEmailFormat(firstName, lastName),
                    Convert.toHungarianPhoneFormatOf(generateElementOf(phones)), gender,
                    generateAddress(), rnd.NextDouble() * 600.0);
            ++Id;
            return employee;
        }
    }
